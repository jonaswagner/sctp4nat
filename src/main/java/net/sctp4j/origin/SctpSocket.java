...
public class SctpSocket {
	...
	private static void debugChunks(byte[] packet) {
		...
		while ((packet.length - offset) >= 4) {
			...
			if (chunkType == 1) {
				...
				logger.debug("ITAG: 0x" + Long.toHexString(initTag) + " a_rwnd: " + a_rwnd + " nOutStream: "
						+ nOutStream + " nInStream: " + nInStream + " initTSN: 0x" + Long.toHexString(initTSN));

				// Parse Type-Length-Value chunks
				/*
				 * while(offset < chunkLength) { //System.out.println(packet[offset++]&0xFF);
				 * int type = bytes_to_short(packet, offset); offset += 2;
				 * 
				 * int length = bytes_to_short(packet, offset); offset += 2;
				 * 
				 * // value offset += (length-4); System.out.println(
				...
				long TSN = bytes_to_long(packet, offset);
				offset += 4;

				int streamIdS = bytes_to_short(packet, offset);
				offset += 2;

				int streamSeq = bytes_to_short(packet, offset);
				offset += 2;

				long PPID = bytes_to_long(packet, offset);
				offset += 4;

				logger.debug("U: " + U + " B: " + B + " E: " + E + " TSN: 0x" + Long.toHexString(TSN) + " SID: 0x"
						+ Integer.toHexString(streamIdS) + " SSEQ: 0x" + Integer.toHexString(streamSeq) + " PPID: 0x"
						+ Long.toHexString(PPID));
	...
	public static void debugSctpPacket(byte[] packet, String id) {
		...
		if (packet.length >= 12) {
			...
			logger.debug("SRC P: " + srcPort + " DST P: " + dstPort + " VTAG: 0x" + Long.toHexString(verificationTag)
	 ...
	 * @param localPort
	 *            local SCTP port on which this socket is bound.
	 */
	 ...
	 * <tt>SctpSocket</tt> has not been closed (yet). Each <tt>lockPtr</tt> method
	 * invocation must be balanced with a subsequent <tt>unlockPtr</tt> method
	 * invocation.
	 *
	 * @return <tt>ptr</tt>
	 * @throws IOException
	 *             if this <tt>SctpSocket</tt> has (already) been closed
	 */
	 ...
	 * @param offset
	 *            the position in the packet buffer where actual data starts
	 * @param len
	 ...
	 *            the data to send.
	 * @param offset
	 *            position of the data inside the buffer
	 * @param len
	 *            data length
	 * @param ordered
	 *            should we care about message order ?
	 ...
	 *
	 * @param link
	 *            <tt>NetworkLink</tt> that will be used by this instance to send
	 *            network packets.
	 */
	...
	/**
	 * Unlocks {@link #ptr} for reading. If this <tt>SctpSocket</tt> has been closed
	 * while <tt>ptr</tt> was locked for reading and there are no other readers at
	 * the time of the method invocation, closes <tt>ptr</tt>. Each
	...
	public interface NotificationListener {
		 ...
		 *
		 * @param socket
		 *            the {@link SctpSocket} notification source.
		 * @param notification
		 *            the <tt>SctpNotification</tt> triggered.
		 */
		public void onSctpNotification(SctpSocket socket, SctpNotification notification);
