 ...
 * 
 * 981 + 25 = 1006
 * 1024 - 1006 = 18 --> where are those 18 Bytes?
 * @author root
 *
 */
public class FileTransmissionTest {
	...
	public void sctpChannelTest() throws InterruptedException {
		...
		Thread client = new Thread(new Runnable() {
			...
			public void run() {
				...
				SctpConnection channel = SctpConnection.builder().local(local).cb(new SctpDefaultConfig().getCb()).remote(remote).build();
...
//						result.send(new byte[982 * 1024], true, 1, 0); this does not work!, there are only 10 streams available per association
					}
				});
